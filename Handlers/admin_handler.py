...
class UploadImageUserHandler(TornadoRequestBase):
    ...
    def post(self, *args, **kwargs):
        ...
        file2 = os.path.join("var", "www", "ftm", "static", "upload", "user_images")
...
class bill_Handler(TornadoRequestBase):
    ...
    def post(self, *args, **kwargs):
        ...
        # partnership1 = Admin_user.select(fn.sum(Admin_user.account)).where(Admin_user.id == user_id).dicts()[0][
        # 'sum(`t1`.`account`)']
        # if not partnership1:
        # partnership1 = 0
        # partnership2 = User.select(fn.sum(User.account)).where(User.id == user_id).dicts()[0]['sum(`t1`.`account`)']
        # if not partnership2:
        # partnership2 = 0
        # Transaction.select(Transaction.amount, Transaction_team.id_name).join(Transaction_team).where(
        # Transaction.payer_id == user_id)
        # sum1 = int(payment) + int(spent) - int(recive) - int(partnership1)
        # sum2 = int(payment) + int(spent) - int(recive) - int(partnership2)
        #
        # if image1:
        # if sum1 > 0:
        # status = "بستانکار"
        ...
        #     dict1 = {'image': image1, 'payment': payment, 'recive': recive, 'spent': spent, 'partnership': partnership1,
        #              'sum': sum1, 'status': status}
        #     self.write(dict1)
        # else:
        #     if sum2 > 0:
        #         status = "بستانکار"
        ...
        #         status = "بدهکار"
        #
        #     dict2 = {'image': image2, 'payment': payment, 'recive': recive, 'spent': spent, 'partnership': partnership2,
        #              'sum': sum2, 'status': status}
...
class del_message_Handler(TornadoRequestBase):
    ...
    def post(self, *args, **kwargs):
        id_message = self.get_argument('id_message')
        # try:
        # find_msg = Message().select().where(Message.id == id_message).get()
        #     find_msg = find_msg.status
        # except:
        #     find_msg = False
        # if find_msg == True:
        #     count = int(self.session.get('count_message'))-1
        ...
        #     count = self.session.get('count_message')
        #     self.session.set('count_message',count)
        try:
